...
module.exports = function (obj, pKey, key, nu, old, dbEvent, postponed) {
	var data;

	// Iterators
	if (obj.hasOwnProperty('__descriptorIterators__')) {
		if (obj.__descriptorIterators__[pKey]) {
			obj.__descriptorIterators__[pKey].forEach(function (iterator) {
	...
	if (obj.hasOwnProperty('__observableDescriptorProperties__')) {
		data = obj.__observableDescriptorProperties__[pKey];
		if (data) {
			data = data[key];
			if (data) {
				data._postponed_ += 1;
