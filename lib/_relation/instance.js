		...
		if ((typeof nu === 'function') && !nu._type_) {
			if (nu.length || this.__external.__value || !this._getter_ ||
				...
				if (!this.__multiple.__value) {
					if (nu != null) nu = this.__ns.__value.normalize(nu);
					else nu = null;
					this.__value = nu;
				} else if (this.__value && this.__value._isSet_ &&
						(this.__value !== getPrototypeOf(this).__value)) {
...
defineProperties(proto, {
	...
	get: d(function (name) {
		if (name in this) {
			if (this['__' + name] && (this['__' + name]._type_ === 'relation')) {
				return this['_' + name];
			}
			if ((('__' + name) in this) || (('_' + name) in this)) { //jslint: skip
				throw new Error("'" + name +
					"' is already defined as non dbjs relation value");
			}
		}
		if (!nameRe.test(name)) {
