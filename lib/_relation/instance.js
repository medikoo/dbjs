...
relation = defineProperties(require('./'), {
	...
	$$create: d(function (obj) {
		...
		value = descriptorCreate.__value.value =
			(this.__value && this.__value._isSet_) ? undefined : this.__value;
		...
		if ((typeof nu === 'function') && !nu._type_) {
			if (nu.length || this.__external.__value || !this._getter_ ||
				...
				if (!this.__multiple.__value) {
					this.__value = nu = this.__ns.__value.normalize(nu);
				} else if (this.hasOwnProperty('__value') && this.__value &&
						this.__value._isSet_) {
