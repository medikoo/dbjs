...
defineProperties(relation, {
	...
	forEach: d(function (cb/*, thisArg*/) {
		var thisArg, index, proto, items, ignores, iterate, ns;
		this._assertSet_();
		callable(cb);
		thisArg = arguments[1];
		index = -1;
		...
		this._forEachItem_(function (item) {
			...
			items.push({ item: item, value: item._subject_,
				order: item.__order.__value });
		});
		if (!this.hasOwnProperty('_value')) {
			...
			iterate = function (item) {
				...
				ignores[item._key_] = true;
				if (!item.__value) return;
				if ((proto.ns !== ns) && (ns.normalize(item._subject_) == null)) {
					return;
				}
				items.push({ item: item, value: item._subject_,
					order: item.__order.__value });
			...
			while (proto && (proto._type_ === 'relation')) {
				...
				proto._forEachItem_(iterate);
				if (proto.hasOwnProperty('_value')) break;
				proto = getPrototypeOf(proto);
			}
		}
		items.sort(byOrder).forEach(function (item, index) {
			call.call(cb, thisArg, item.value, item.item, this, index);
		}, this);
	}),
