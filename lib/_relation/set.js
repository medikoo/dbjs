...
defineProperties(relation, {
	...
	$delete: d(function (value) {
		var item, key, ns, existing, protoSet;
		...
		key = ns._serialize_(value);
		if (!key) return null;
		existing = Boolean(this[key]);
		protoSet = !this.hasOwnProperty('_value') &&
			Boolean(getPrototypeOf(this)[key]);
		item = this._getItem_(key, value);
		if (!existing || protoSet) {
			item._signal_(false);
			return item;
		}
		item.$delete();
	...
	_setExtensions_: d({ _serialize: d(serialize) }),
	_clear_: d(function () {
