...
defineProperties(relation, {
	...
	$add: d(function (value) {
		...
		item = this.hasOwnProperty(key) ? this[key] :
				this._setItem_.$$createItem(key, value);
		item.$$setValue(true);
		item._signal_(true);
		return item;
	}),
	_getSetItem_: d(function (key, value) {
		if (!this.hasOwnProperty(key)) {
			if (value == null) value = unserialize(key);
			return this._setItem_.$$createItem(key, value);
		}
		return this[key];
	}),
		...
		if (isArray(value)) {
			if (!value.length) {
				...
				forEach(map, function (value, key) {
					var item = this._getSetItem_(key, value.value);
					item._signal_(true);
			...
			item = this._getSetItem_(key, value);
			item._signal_(true);
