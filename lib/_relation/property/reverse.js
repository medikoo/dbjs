...
var d           = require('es5-ext/lib/Object/descriptor')
  ...
  , rel, setReverseRoot, setReverse, validate, getReverse;
...
setReverseRoot = function (rel, nu, index) {
	var old = rel.__value;
	if (old !== nu) {
...
defineProperties(rel, {
	$$create: d(function (obj) {
		...
		obj._ns.on('change', function (nu, old) {
			...
			if ((old._childType_ === 'object') && (old._id_ !== 'Object')) {
				old.prototype['_' + rel.__value]._update_();
			}
			if ((nu._childType_ === 'object') && (nu._id_ !== 'Object') &&
					((index = rel.obj._selfIndex_))) {
				setReverseRoot(nu.prototype.get(rel.__value), getReverse(index), index);
			}
		});
		return rel;
	}),
	__update_: d(function () {
		...
		if ((ns._childType_ === 'object') && (ns._id_ !== 'Object#')) {
			// Remove old reverse
			if (old && (nu !== old)) ns.prototype['_' + old]._update_();

			// Set new reverse
			if (nu && ((index = this.obj._selfIndex_))) {
				setReverseRoot(ns.prototype.get(nu), getReverse(index), index);
			}
		}
