...
setReverse = function (rel, nu, index) {
	var old = rel.__value;
	if (old !== nu) {
	...
	$$create: d(function (obj) {
		...
		obj._ns.on('change', function (nu, old) {
			...
			if ((nu._childType_ === 'object') && (nu._id_ !== 'Object') &&
				...
				setReverse(nu.prototype.get(rel.__value), getReverse(index), index);
			}
		});
		return rel;
	}),
	__update_: d(function () {
		...
		if ((ns._childType_ === 'object') && (ns._id_ !== 'Object#')) {
			...
			if (nu && ((index = this.obj._selfIndex_))) {
				setReverse(ns.prototype.get(nu), getReverse(index), index);
			}
		}
