...
item = defineProperties(proto.$$create(':"'), {
	...
	__update_: d(function (propagate) {
		var rel = this.obj, old = this.__value, nu, lastValue;
		if (this.obj.ns.normalize(this._subject_) == null) {
			nu = false;
		} else if (this._value != null) {
			nu = this._value;
		} else {
			while (!rel.hasOwnProperty('_value')) {
				if ((rel = getPrototypeOf(rel))._type_ !== 'relation') {
					nu = false;
					break;
				} else if (!rel._isSet_) {
					nu = false;
					break;
				} else if (rel.hasOwnProperty(this._key_)) {
					nu = rel[this._key_].__value;
					break;
				}
			}
			if (nu == null) nu = false;
		...
		rel._children_.forEach(function (child) {
			...
			if (child.hasOwnProperty(this._key_)) {
				...
				return;
					...
					(child.ns.normalize(this._subject_) == null)) {
				return;
			}
