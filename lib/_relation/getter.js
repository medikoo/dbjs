...
var arrRemove = require('es5-ext/lib/Array/prototype/remove')
  ...
  , activateTriggers, deactivateTriggers, isProto;

isProto = function (obj) {
	return (obj._type_ === 'object') && (last.call(obj._id_) === '#');
};
...
activateTriggers = function (baseRel) {
	...
	add = function (obj) {
		...
		baseRel.triggers.forEach(function (triggerName) {
			obj._getRel_(triggerName).on('change', rel._update_);
		});
		rel._update_();
	...
	baseRel.triggers.forEach(function (triggerName) {
		...
		obj._getRel_(triggerName).on('change', baseRel._update_);
	});
