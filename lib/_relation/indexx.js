...
RevRel.prototype = ee(Object.create(ReadOnlySet.prototype, {
	...
	values: d.gs(function () {
		return keys(this).map(function (key) { return this[key]; }, this);
	}),
	_add: d(function (obj) {
		++this.count;
...
defineProperties(ee(Index.prototype), {
	...
	delete: d(function (key, obj) {
		...
		rev = this[key];
		if (!rev) {
			console.log("Error data", key, obj);
			console.error("Could not find reverse map!");
			return;
		}
		if (isUnique) old = rev._single;
		rev._delete(obj);
...
Object.defineProperties(relation, {
	...
	indexFilter: d(function (value) {
		...
		if (!index) {
			throw customError("Filter not supported", 'INDEX_NOT SUPPORTED');
		}
		key = getIndexKey(value);
		if (key == null) {
			throw customError("Wrong filter value", 'WRONG_FILTER_VALUE');
		}
