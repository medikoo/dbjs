...
var extend      = require('es5-ext/lib/Object/extend')
  ...
  , Fragment, isTrackableObj;

isTrackableObj = function (obj) {
	return (obj && (obj._type_ === 'object') && (last.call(obj._id_) !== '#')) ||
			false;
};
...
ee(extend(Fragment.prototype, {
	...
	offObj: function (obj) {
		...
		if (fragment.refCount === 1) {
			...
			forEach(objs, function (obj) { this.emit('remove', obj); }, this);
	...
	onAssign: function (rel) {
		var obj = rel.obj;
		if (!isTrackableObj(obj) || !this.approveRelation(rel)) return;
		this.onObj(obj);
	},
	...
	onRelation: function (event, fragment) {
		...
		// Process new
		if (isTrackableObj(value) && (this.valueRels[relId] !== value) &&
				(value !== fragment.obj)) {
	...
	onSetItem: function (event, fragment) {
		...
		if (isTrackableObj(value)) {
			if (added) this.offObj(value);
			else this.onObj(value);
		}
		this.emit('update', event);
	}
}));
