...
var d                = require('es5-ext/lib/Object/descriptor')
  ...
  , alwaysPass = function (obj) { return true; }
  , getId = function (obj) { return obj._id_; }
  , getId2 = function (rel, val) { return rel._id_ + '\n' + val._id_; }
  , Observer, passTopRelations, getRelPass, getValPass;
...
ee(Object.defineProperties(Observer.prototype, extend({
	...
	onAssign: d(function (rel) {
		...
		this._extend(obj,
			pass.relation ? getRelPass(callable(pass.relation)) : this.passRelation,
			pass.value ? getValPass(callable(pass.value)) : this.passValue,
			pass.assignment ? getValPass(callable(pass.assignment)) :
					this.passAssignment);
	}),
	...
	onRelation: d(function (event) {
		...
		if (this.valueRels[relId] && (this.valueRels[relId] !== value)) {
			...
			this._extend.clearRef(this.valueRels[relId],
				pass.relation ? getRelPass(pass.relation) : this.passRelation,
				pass.value ? getValPass(pass.value) : this.passValue,
				pass.assignment ? getValPass(pass.assignment) : this.passAssignment);
			...
			if (added) {
				if (!this.valueRels[itemId]) {
					...
					this._extend(this.valueRels[itemId] = value,
						pass.relation ? getRelPass(callable(pass.relation)) :
								this.passRelation,
						pass.value ? getValPass(callable(pass.value)) : this.passValue,
						pass.assignment ? getValPass(callable(pass.assignment)) :
								this.passAssignment);
...
module.exports = function (obj/*, pass*/) {
	...
	root = new Multi();
	observer = new Observer(root, obj,
		pass.relation ? getRelPass(callable(pass.relation)) : alwaysPass,
		pass.value ? getValPass(callable(pass.value)) : alwaysPass,
		pass.assignment ? getValPass(callable(pass.assignment)) : alwaysPass);
