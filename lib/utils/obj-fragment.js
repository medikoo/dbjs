...
ee(extend(ObjFragment.prototype, {
	...
	onRelation: function (obj) {
		...
		if ((event = obj._lastEvent_)) {
			this.objects[id] = obj;
			this.emit('relupdate', event, null, this);
		}
	...
	onSetItem: function (obj) {
		...
		if ((event = obj._lastEvent_)) {
			...
			this.emit('setitemupdate', event, null, this);
		}
		obj._forEachRelation_(this.onRelation, this);
	},
	onUpdate: function (event, last) {
		...
		if (obj._type_ === 'relation') {
			if (passTree(obj)) {
				...
				else this.emit('relupdate', event, last, this);
			}
		} else {
			rel = obj.obj;
			if (passTree(rel)) {
				...
				else this.emit('setitemupdate', event, last, this);
			}
		}
	}
}));
