...
define = function (obj, name, property, attr) {
	var pname = '_' + name;
	defineProperty(obj, pname, d('c', property));
	defineProperty(obj, name, d.gs(attr || '', function () {
		var prop = this[pname];
		if (!prop.ns) return prop.value;
		if (prop.ns === prop.value) {
			return this.hasOwnProperty(pname) ? prop.value : undefined;
		}
		return (prop.value == null) ? null : prop.ns.normalize(prop.value);
	}, function (value) {
		if (this.hasOwnProperty(pname)) return this[pname].set(value);
		defineProperty(this, pname, d('c', this[pname].create(value)));
