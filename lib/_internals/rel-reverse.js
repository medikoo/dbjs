...
exports.add = function (rel, value) {
	...
	obj = rel.obj;
	data[obj._id_ + ':' + rel.name] = obj;

	proto = getPrototypeOf(rel);
	while (proto.obj) {
		relId = proto.obj._id_ + ':' + proto.name;
		rev = data[relId];
...
exports.remove = function (rel, value) {
	...
	data = reverse[id];
	obj = rel.obj;
	delete data[obj._id_ + ':' + rel.name];

	proto = getPrototypeOf(rel);
	while (proto.obj) {
		data[proto.obj._id_ + ':' + proto.name]._remove(obj);
		proto = getPrototypeOf(proto);
	}
};

exports.set = function (rel, name) {
	var ns = rel.ns, relId = rel.obj._id_ + ':' + rel.name;
...
exports.unset = function (rel, name) { delete rel.ns.prototype[name]; };
