...
addTriggers = function (rel) {
	...
	rel.triggers.forEach(onadd = function (name) {
		if (!rel.obj['_' + name]) define(rel.obj, name);
		rel.obj['_' + name].on('update', rel._refresh_)
	});
	rel.triggers.on('add', onadd);
	rel.triggers.on('remove', onremove = function (name) {
		rel.obj['-' + name].off('update', rel._refresh_);
	});
	rel._triggersSetup_ = { onadd: onadd, onremove: onremove };
};

removeTriggers = function (rel) {
	var setup = rel.hasOwnProperty('_triggersSetup_') && rel._triggersSetup_;
	if (!setup) return;
	rel.triggers.forEach(function (name) {
		rel.obj['_' + name].off('update', rel._refresh_)
	});
	rel.triggers.off('add', setup.onadd);
	rel.triggers.off('remove', setup.onremove);
