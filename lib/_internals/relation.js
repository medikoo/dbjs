...
addTriggers = function (rel) {
	...
	rel.triggers.on('add', ontriggeradd = function (triggerName) {
		...
		objects.forEach(function self(obj) {
			var rel = obj[name];
			obj[triggerName].on('update', rel._refresh_);
			rel._refresh_();
		});
	});
	rel.triggers.on('delete', ontriggerdelete = function (triggerName) {
		...
		objects.forEach(function self(obj) {
			var rel = obj[name];
			obj[triggerName].off('update', rel._refresh_);
			rel._refresh_();
		});
	});
...
refresh = function () {
	var nu = this._value, old = this.__value, name, obj;
		...
		if (nu.length || (!this.triggers.count && !this.fixedValue)) {
			...
			if (this.hasOwnProperty('_value')) {
				if (this.fixedValue) {
					...
					addTriggers(this);
				} else if (this._triggersSetup_.objects) {
					name = '_' + this.name;
					obj = this.obj;
					this._triggersSetup_.objects.forEach(function (child) {
						if (obj === child) return;
						child[name]._refresh_();
					});
				} else {
					removeTriggers(this);
					addTriggers(this);
					...
					!this.hasOwnProperty('_triggersSetup_')) {
