...
module.exports = exports = function (sourceId, stamp, objId, value) {
	...
	if (signal.hasOwnProperty(objId) && (signal[objId] === value)) return;

	signal[objId] = value;
	signal._order.push(value);
...
exports.fromString = function (str) {
	...
	if (isItemId(str[0])) {
		...
		exports(data[0], data[1], str.slice(0, i), str.slice(i + 2));
		return;
	}

	// '.' within value
	exports(data[0], data[1], data[2], str.slice(data[2].length + 1));
};
