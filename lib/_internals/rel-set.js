...
defineProperties(RelationSet.prototype, {
	...
	remove: d('c', function (value) {
		...
		if (this.propertyIsEnumerable(key)) {
			...
			if (isTopValue && this.rel._required && (this.__count === 1)) {
	...
	forEach: d('c', function (cb/*, thisArg*/) {
	...
	getItemProperties: d('c', function (value) {
		var key;
	...
	_reset: d('c', function (value) {
		var ns, key, map, last;
		if (value === undefined) {
			this._clear();
			delete this.rel._value;
			return;
		}
		if (value === null) {
			this._clear();
		...
		if (this.rel._multiple && isArray(value)) {
			if (!value.length) {
				this._clear();
				this.rel._value = null;
				return;
