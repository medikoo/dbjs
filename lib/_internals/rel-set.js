...
defineProperties(RelationSet.prototype, {
	...
	add: d(function (value) {
		...
		defineProperty(this, key, d('ce', value));
	...
	remove: d(function (value) {
		...
		key = getKey(value);
		if (this.rel.required && this.rel.hasOwnProperty('_value') &&
				(this.__count === 1) && (this.rel._value === value)) {
			throw new TypeError('Cannot remove the only value');
		}
		if (this.propertyIsEnumerable(key)) --this.__count;
		defineProperty(this, key, d('c', null));
		if (this.rel.hasOwnProperty('_value') && (this.rel._value === value)) {
			this.rel._value = this.last;
		}
	}),
	...
	forEach: d(function (cb/*, thisArg*/) {
		var thisArg = arguments[1], index = -1, proto
		  , ignores, addToIgnores, setIgnore;
		callable(cb);
		keys(this).forEach(function (key) {
			cb.call(thisArg, this[key], ++index, this);
		}, this);
		...
		proto = getPrototypeOf(this.rel);
		addToIgnores = function (key) {
			if (!ignores[key]) cb.call(thisArg, proto.__set[key], ++index, this);
		};
		while (proto) {
			if (!proto.multiple) {
				if ((proto._value == null) || ignores[getKey(proto._value)]) return;
				cb.call(thisArg, proto._value, ++index, this);
				return;
			}
			if (proto.hasOwnProperty('__set')) {
				keys(proto.__set).forEach(addToIgnores, this);
			}
	...
	_clear: d(function () {
		keys(this).forEach(function (name) {
			defineProperty(this, name, d('c', null));
		}, this);
		this.__count = 0;
	}),
	_reset: d(function (value) {
		...
		keys(this).forEach(function (lkey) {
			if (lkey !== key) defineProperty(this, lkey, d('c', null));
				else (key = null);
		}, this);
		if (key) defineProperty(this, key, d('ce', value));
