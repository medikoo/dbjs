...
defineProperties(signals, {
	_emit: d(emit = function (signal) {
		signal._order.forEach(function (id) {
			var i, l, current, obj, value, prevSignal, prevValue, importId;
			value = signal[id];
			importId = (signal._importSourceMap && signal._importSourceMap[id]) ||
				null;
			obj = getObject(id);
			prevSignal = history[id][1];
			if (prevSignal) {
				prevValue = prevSignal[id];
				if (prevValue && prevValue._id_) {
					signals.emit('-' + prevValue._id_, obj, signal, importId);
				}
			}
			if (value && value._id_) {
				signals.emit('+' + value._id_, obj, signal, importId);
			}
			signals.emit(id, obj, value, signal, importId);
			id = idSplit(id);
			l = id.length;
			if (l !== 1) {
				current = id[0];
				for (i = 1; i < l; ++i) {
					signals.emit(current + '*', obj, value, signal, importId);
					current += ':' + id[i];
				}
			}
