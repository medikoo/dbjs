...
objects['Base#'] = baseProto = Base.prototype = create(proto, {
	...
	$$create: d(function (id) {
		...
		isSet = this._type_ === 'prototype';
		if (isSet) defineProperty(obj, '_type_', d('object'));
		ns = this.ns;
		while (ns._type_ && (ns !== Base)) {
			ns[id] = obj;
			if (isSet && (ns._childType_ === 'object')) ++ns._count_;
			ns = getPrototypeOf(ns);
		}
		return obj;
	}),
	$$setValue: d(function (nu) {
		var ns, old, id, isObject;
		if (!nu) nu = baseProto;
		...
		ns = old.ns;
		id = this._id_;
		isObject = this._type_ === 'object';
		while (ns._type_ && (ns !== Base)) {
			delete ns[id];
			if (isObject && (ns._childType_ === 'object')) --ns._count_;
			ns = getPrototypeOf(ns);
		}
		ns = nu.ns;
		while (ns._type_ && (ns !== Base)) {
