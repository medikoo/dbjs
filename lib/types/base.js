...
setItemProto = function (item) {
	var old = getPrototypeOf(item), nu = this._setItem_;
...
setRelationProto = function (rel) {
	...
	// Fix indexing
	if (rel.hasOwnProperty('__index_')) {
		rel.__index_.setParent(nu._selfIndex_);
	} else if ((key = getIndexKey(rel.__value))) {
	...
	if (rel.hasOwnProperty('__setItem_')) setItemProto.call(nu, rel.__setItem_);
