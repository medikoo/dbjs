...
var i                = require('es5-ext/lib/Function/i')
  ...
  , Base, Plain, Relation, RelTransport, define, abstractLock, getConstructor;
...
defineProperties(Base, {
	...
	create: d('c', function (name, construct, nsProps, objProps) {
		...
		error = this.combineErrors(
			construct && this._construct.validate(construct),
			nsProps && this.validatePropertiesNew(nsProps),
	...
	$create: d('c', function (name, construct) {
		...
		if (construct) constructor._construct.$set(construct);
