...
var copy        = require('es5-ext/lib/Array/prototype/copy')
  ...
  , InterSet    = require('../_proto/set-intersection')
  ...
  , ObjectType, proto, rel, byCreatedAt, setExtensions;

require('memoizee/lib/ext/method');

byCreatedAt = function (a, b) { return a._lastModified_ - b._lastModified_; };

setExtensions = extend({
	listByCreatedAt: d(function () { return this.list(byCreatedAt); })
}, memoize(function (name) {
	var rel, list, compare, index;
	name = String(name);
	if (!nameRe.test(name)) throw new Error(name + " is invalid name");
	rel = this.obj.prototype.get(name);
	compare = this.obj.compare;
	list = this.list(function (a, b) { return compare(a[name], b[name]); });
	index = rel._selfIndex_;
	if (index) index.on('update', list._sort);
	return list;
}, { method: 'listByProperty' }));

module.exports = ObjectType = defineProperties(Base.$$create('Object'), extend({
	...
	_setExtensions_: d(setExtensions)
}, setExtensions));
setExtensions._serialize = d(ObjectType._serialize_);
FilterSet.defineOn(ObjectType);
ObjectList.defineOn(ObjectType);
InterSet.defineOn(ObjectType);
